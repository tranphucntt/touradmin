...
.img_close {
	...
	<?php
		$file_no="no_img.jpg";           
		$urlfilebasic = JURI::root() . 'images/files/filesmd5/';
		if($this->item->file){
			$filedisplay=$this->item->file;
			$filevalue=$this->item->file;
		}else{
			$filedisplay=$file_no;
			$filevalue='';
		}
	...
	<div class="file_preview">
		<a target="_blank" id="returnuploadfile" href="<?php echo $urlfilebasic.$this->item->file; ?>">
		...
		<input type="hidden" name="jform[file]" value="<?php echo $filevalue; ?>" id="filevalue" />
	</div>
	...
	<script>
     jQuery(document).ready(function()
                        ...
                        jQuery("#path_to_file_file").change(function(event){
                        	 if(jQuery("#filevalue").val()!=''){
                         		alert("<?php echo JText::_('Delete file'); ?>");
		                                    ...
		                                    jQuery.ajax({
		                                        url: "<?php echo JURI::root() . 'administrator';?>/index.php?option=com_bookpro&view=ajaxfile&task=uploadfile&tmpl=component&table=<?php echo isset($this->item->table)?$this->item->table:null; ?>&id=<?php echo isset($this->item->id)?$this->item->id:null; ?>",
		                                        type: "POST",
		                                        data: data,
		                                        cache: false,
		                                        processData: false,
            ...
            function closeImageFile()
            {
                ...
                jQuery.ajax({
                    url: "<?php echo JURI::root() . 'administrator';?>/index.php?option=com_bookpro&view=ajaxfile&task=remove&tmpl=component&table=<?php echo isset($this->item->table)?$this->item->table:null; ?>&id=<?php echo isset($this->item->id)?$this->item->id:null; ?>",
                    type: "POST",
                    data: data,
                    cache: false,
                    processData: false,
